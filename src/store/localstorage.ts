...
export const themeColor = '#9c5bd9';
export const iconSize = useStorage('icones-icon-size', 'text-4xl');
export const previewColor = ref('##D3D3D3');
...
export const preferredCase = useStorage<IdCase>(
  'icones-preferfed-case',
  'iconify'
);
export const sortAlphabetically = useStorage(
  'icones-alpha-sort-collections',
  false
);
...
export function pushRecent(id: string) {
  recentIds.value = [id, ...recentIds.value.filter((i) => i !== id)].slice(
    0,
    RECENT_CAPACITY
  );
}
...
export function toggleFavorite(id: string) {
  ...
  if (index >= 0) favoritedIds.value.splice(index, 1);
  else favoritedIds.value.push(id);
}

export function addToBag(id: string) {
  if (!bags.value.includes(id)) bags.value.push(id);
}

export function removeFromBag(id: string) {
  const index = bags.value.indexOf(id);
  if (index >= 0) bags.value.splice(index, 1);
}
